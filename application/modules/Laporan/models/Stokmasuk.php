...
class Stokmasuk extends CI_Model {
	...
	public function MasukList($param , $paging) {
		...
		if(!empty($param["no_penerimaan"])) {
			$flt = "no_penerimaan = '".$param["no_penerimaan"]."'";
			$filter .= empty($filter) ? $flt : " and ".$flt;
		}
		...
		if(!empty($param['date_range'])){
			...
			$flt = "tgl_masuk BETWEEN '".date("Y-m-d",strtotime($param['date_range']['awal']))."' and '".date("Y-m-d",strtotime($param['date_range']['akhir']))."' ";
			$filter .= empty($filter) ? $flt : " and ".$flt;
		...
		if(!empty($param["Sort"]))  $data["ParamsSort"] = "id desc";
